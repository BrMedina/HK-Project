...
export default function ManualRateCard({ customRateInput, saving, onRateChange, onSave }: Props) {
  ...
  return (
    <View style={[s.card, { backgroundColor: "#39baa6", shadowColor: "#39baa6" }]}>
      <View style={s.decor}>
        <Settings size={100} color="#fff" style={{ opacity: 0.08 }} />
      </View>

      <Text style={s.title}>Manual Rate Setup</Text>
      <Text style={[s.desc, { color: "rgba(255,255,255,0.8)" }]}>Override the current market rate for personal calculations.</Text>

      <View style={[s.inputRow, { backgroundColor: "rgba(255,255,255,0.08)", borderColor: "rgba(255,255,255,0.15)" }]}>
        <Text style={s.label}>1 HKD = </Text>
        <View style={[s.inputWrap, { borderBottomColor: "rgba(255,255,255,0.4)" }]}>
          <TextInput
            style={s.input}
      ...
      <Pressable style={[s.saveBtn, saving && { opacity: 0.7 }]} onPress={onSave} disabled={saving}>
        {saving
          ? <ActivityIndicator size="small" color="#39baa6" />
          : <Text style={[s.saveBtnText, { color: "#39baa6" }]}>SAVE CUSTOM RATE</Text>}
      </Pressable>
    </View>
  );
}
